...
parser = BaseOptions()
# tạo ra một mesh có màu sắc phù hợp từ dữ liệu đầu vào và mạng neural network đã được huấn luyện.
def gen_mesh(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        ...
        pass
    # b_min và b_max là các giá trị tối thiểu và tối đa của bounding box của mesh.
    b_min = data['b_min']
    b_max = data['b_max']
    try:
        save_img_path = save_path[:-4] + '.jpg'
        save_img_list = []
...
def gen_mesh_imgColor(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        ...
        # nếu điều này trả về lỗi, phép chiếu phải được xác định ở một nơi khác
        # Tính toán màu sắc cho mesh
